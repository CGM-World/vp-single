...
class Absorber:
    def __init__(self, con, ATOM, trans, vel_range, Inst, seed, snr, zabs, v, logN, b):
        ...
        self.waves_os = vel_to_wave(con, self.vels_os, self.atom["wave_cen"], self.zabs)    # oversampled waves
        self.tau = self.get_tau(con, self.zabs, v, logN, b)                                 # optical depth
        self.f_norm_ = np.exp(-self.tau)                                            # normalized flux before convolution
    ...
    def convolve_with_isf(self, Inst):
        ...
        conv = np.concatenate((pad, np.convolve(self.f_norm_, Inst.gauss_kernel, mode="valid"), pad))
